...
function draw() {
    ...
    if (!levelComplete) {
        ...
        collectibles.forEach(collectible => {
            collectible.display();
            // Check if player hits the collectible
            if (dist(player.position.x, player.position.y, player.position.z, collectible.position.x, collectible.position.y, collectible.position.z) < (player.size + collectible.size)) {
                collectible.collected = true;
            }
        });

        collectibles = collectibles.filter(collectible => !collectible.collected); // Remove collected spheres
